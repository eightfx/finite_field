	...
	fn add(self, other: FiniteField) -> FiniteField{
		match self.element {
			Element::PrimeField(_) =>{
				...
				let mut result = (x + y) % self.char as NumType;
				if result < 0 {
					result += self.char as NumType;
				}
				else if result == self.char as NumType {
					result = 0;
				}

				FiniteField{
					char:self.char ,
					element: Element::PrimeField(result),
				}
			}
...
fn main() {
	...
	// let element2:Element = Element::Polynomial(vec![1,2,3,1,2]);
	
	let element1:Element = Element::GaloisField(vec![4,2,3],vec![2,2,3]);
	let element2:Element = Element::GaloisField(vec![3,3,3,4],vec![2,2,3]);
